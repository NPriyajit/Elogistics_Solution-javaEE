...
<body>
 ...
 String  orderno,datedet,vendname,locality,city,state,slno,prodcode,prodname,prodcat,price,quant,amt,btnup,btndel;
 Connection conn;
...
datedet=request.getParameter("date");
 ...
 try{
	 ...
	 stmtSave.executeUpdate("update tblorderinfo set datedet='"+datedet+"',vendname='"+vendname+"',locality='"+locality+"',city='"+city+"',state='"+state+"',slno='"+slno+"',prodcode='"+prodcode+"',prodname='"+prodname+"',prodcat='"+prodcat+"',price='"+price+"',quant='"+quant+"',amt='"+amt+"' where orderno='"+orderno+"'");
	 out.println("<table style=' align:center; padding:60px; margin-left:70px;\'>");
		out.println("  <tr><td ><img src='./images/added.gif' style='width:482px; left:50px;  height:400px;\'></td><td><p style=' font-family: Courier, monospace; font-weight:500; font-size:20px; position: relative; margin-left:-60px;'>Data Updated successfully</p></td></tr>");
		 out.println("<tr><td></td><td></td><td></td>");
		  out.println("  <td><button style='border-radius:4px; font-size:20px;'  onclick='window.open(\"orderinfofc.jsp\",\"_SELF\")'>Back</button></td>");
...
else if(btndel != null)
{
	...
	try
	{
		...
		stmtdel.executeUpdate("delete from tblorderinfo where orderno='"+orderno+"' ");
		out.println("<table style=' align:center; padding:60px; margin-left:70px;\'>");
