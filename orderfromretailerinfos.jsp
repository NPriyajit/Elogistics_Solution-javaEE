...
<body>
 ...
 <%!
 Statement stmtSave;
 String  orderno,odate,retname,locality,city,state,slno,prodcode,prodname,prodcat,price,quant,amt;
 Connection conn;
...
<%
 orderno=request.getParameter("txtordno");
odate=request.getParameter("odate");
 ...
 try{
	 ...
	 stmtSave.executeUpdate("insert into tblorderretailinfo values('"+orderno+"','"+odate+"','"+retname+"','"+locality+"','"+city+"','"+state+"','"+slno+"','"+prodcode+"','"+prodname+"','"+prodcat+"','"+price+"','"+quant+"','"+amt+"')");
  out.println("<table style=' align:center; padding:60px; margin-left:70px;\'>");
    out.println("  <tr><td ><img src='./images/submit.gif' style='width:582px; left:50px;  height:470px;\'></td><td><p style=' font-family: Courier, monospace; font-weight:500; font-size:20px; position: relative; margin-left:-60px;'>Data Added successfully</p></td></tr>");
     out.println("<tr><td></td><td></td><td></td>");
      out.println("  <td><button style='border-radius:4px; font-size:20px;'  onclick='window.open(\"orderfromretailerinfoc.jsp\",\"_SELF\")'>Back</button></td>");
