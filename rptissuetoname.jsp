...
<body>
    ...
    <%!
    Connection conn;
    Statement stmtfill,stmtshow,stmtfill2;
    ResultSet rsfill,rsshow,rsshow2;
      String  orderno , odate , retname , locality , city , state , slno , prodcode , prodname , prodcat , price , quant , amt , idate ,invno;
      String  disper , disamt , cgst , sgst , taxamt , netamt ,sretname,username;
    int i,sl;
 ...
 username=(String)session.getAttribute("ssun");
     ...
     doConnect();
        try{
            ...
            while(rsshow2.next())
            {
                sretname=rsshow2.getString("retname");
            }
        }
        catch(SQLException e)
        {
...
%>
      ...
      <h3   style="text-align:center;padding-bottom:15px;">Issue Of Goods [<%=sretname %>] &nbsp;<sup><small class="text-muted">Report</small></sup></h3>
    <div class="well" style="border-top: solid black;border-bottom: solid black ;" >
      ...
      <table  style="margin-top:40px; border:2px solid black; font-size:12px;"class="table ">
        ...
        try{
            ...
            rsshow=stmtfill.executeQuery("select * from tblissuegoodsinfo where retname='"+sretname+"'");
            i=0;
            sl=0;
            while(rsshow.next())
            {
